  ...
  return (
    <Pressable
      ...
      style={({ pressed, hovered }) => {
        const state = isDisabled ? 'disabled' : pressed || hovered ? 'active' : 'default';
    ...
    >
      {({ pressed, hovered }) => {
        ...
        const iconElementColor = loading ? theme.colors.textDisabled : visual.iconColor;

        return (
          <View style={[styles.content, { gap: sizeToken.gap }]}>
            {loading ? (
              <ActivityIndicator size="small" color={visual.textColor} />
            ) : (
              <>
                {leftIcon ? (
...
function renderIcon(icon: IconElement | undefined, color: string, size: number): React.ReactNode {
